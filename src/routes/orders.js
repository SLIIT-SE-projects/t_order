...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    // Enrich every item sequentially to avoid rate-limit bursts on the Product Service
    for (const order of orders) {
      ...
      for (const item of order.items) {
        try {
          ...
          enrichedItems.push({ ...item, imageUrl: data.product?.imageUrl || '' });
        } catch {
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    const order = await Order.findById(req.params.id).lean(); // plain JS object — safe to mutate
    if (!order) return res.status(404).json({ error: 'Order not found' });
    if (order.userId !== req.user.userId && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Access denied' });
    }

    // Enrich each item sequentially to avoid rate-limit bursts on the Product Service
    const enrichedItems = [];
    for (const item of order.items) {
      try {
        const { data } = await axios.get(
